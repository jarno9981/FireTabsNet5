...
namespace FireTabsNet5
{
	...
	/// </summary>
	public abstract partial class FireTitle : Form
	{
		/// <summary>
		/// Event delegate for <see cref="FireTitle.TabDeselecting" /> and <see cref="FireTitle.TabSelecting" /> that allows subscribers to cancel the
		/// event and keep it from proceeding.
		/// </summary>
		/// <param name="sender">Object for which this event was raised.</param>
		/// <param name="e">Data associated with the event.</param>
		public delegate void TitleBarTabCancelEventHandler(object sender, FireTitleTabCancelEventArgs e);

		/// <summary>Event delegate for <see cref="FireTitle.TabSelected" /> and <see cref="FireTitle.TabDeselected" />.</summary>
		/// <param name="sender">Object for which this event was raised.</param>
		/// <param name="e">Data associated with the event.</param>
		public delegate void TitleBarTabEventHandler(object sender, FireTitleTabEventArgs e);
		...
		protected internal FireTitleTabsOverlay _overlay;
		...
		/// </summary>
		protected FireTitleTab _previousActiveTab = null;
		...
		protected FireBaseTabRenderer _tabRenderer;

		/// <summary>List of tabs to display for this window.</summary>
		protected ListWithEvents<FireTitleTab> _tabs = new ListWithEvents<FireTitleTab>();
		...
		public bool AeroPeekEnabled
		{
			...
			set
			{
				...
				if (!_aeroPeekEnabled)
				{
					foreach (FireTitleTab tab in Tabs)
					{
		...
		/// <summary>List of tabs to display for this window.</summary>
		public ListWithEvents<FireTitleTab> Tabs
		{
			get
			{
				return _tabs;
		...
		public FireBaseTabRenderer TabRenderer
		{
			get
			{
				return _tabRenderer;
		...
		public FireTitleTab SelectedTab
		{
			get
			{
				return Tabs.FirstOrDefault((FireTitleTab t) => t.Active);
			}
		...
		public int SelectedTabIndex
		{
			get
			{
				return Tabs.FindIndex((FireTitleTab t) => t.Active);
			}

			set
			{
				FireTitleTab selectedTab = SelectedTab;
				int selectedTabIndex = SelectedTabIndex;

				if (selectedTab != null && selectedTabIndex != value)
				{
					// Raise the TabDeselecting event
					FireTitleTabCancelEventArgs e = new FireTitleTabCancelEventArgs
					{
		...
		public FireTitleApplicationContext ApplicationContext
		{
			get;
			internal set;
		}
		...
		{
			base.OnLoad(e);
			_overlay = FireTitleTabsOverlay.GetInstance(this);
		...
		/// <summary>
		/// Callback that should be implemented by the inheriting class that will create a new <see cref="FireTitleTab" /> object when the add button is
		/// clicked.
		/// </summary>
		/// <returns>A newly created tab.</returns>
		public abstract FireTitleTab CreateTab();

		/// <summary>Callback for the <see cref="TabClicked" /> event.</summary>
		/// <param name="e">Arguments associated with the event.</param>
		protected internal void OnTabClicked(FireTitleTabEventArgs e)
		{
			if (TabClicked != null)
			{
				TabClicked(this, e);
		...
		/// <summary>
		/// Callback for the <see cref="TabDeselecting" /> event.  Called when a <see cref="FireTitleTab" /> is in the process of losing focus.  Grabs an image of
		/// the tab's content to be used when Aero Peek is activated.
		/// </summary>
		/// <param name="e">Arguments associated with the event.</param>
		protected void OnTabDeselecting(FireTitleTabCancelEventArgs e)
		{
		...
		protected void UpdateTabThumbnail(FireTitleTab tab)
		{
		...
		/// <param name="e">Arguments associated with the event.</param>
		protected void OnTabDeselected(FireTitleTabEventArgs e)
		{
		...
		/// <param name="e">Arguments associated with the event.</param>
		protected void OnTabSelecting(FireTitleTabCancelEventArgs e)
		{
		...
		/// <summary>
		/// Callback for the <see cref="TabSelected" /> event.  Called when a <see cref="FireTitleTab" /> gains focus.  Sets the active window in Aero Peek via a
		/// call to <see cref="TabbedThumbnailManager.SetActiveTab(Control)" />.
		/// </summary>
		/// <param name="e">Arguments associated with the event.</param>
		protected void OnTabSelected(FireTitleTabEventArgs e)
		{
		...
		/// <summary>
		/// Handler method that's called when Aero Peek needs to display a thumbnail for a <see cref="FireTitleTab" />; finds the preview bitmap generated in
		...
		{
			foreach (
				FireTitleTab rdcWindow in Tabs.Where(rdcWindow => rdcWindow.Content.Handle == e.WindowHandle && _previews.ContainsKey(rdcWindow.Content)))
			{
		...
		/// <summary>
		/// Callback for the <see cref="Control.ClientSizeChanged" /> event that resizes the <see cref="FireTitleTab.Content" /> form of the currently selected
		...
		/// <summary>Resizes the <see cref="FireTitleTab.Content" /> form of the <paramref name="tab" /> to match the size of the client area for this window.</summary>
		/// <param name="tab">Tab whose <see cref="FireTitleTab.Content" /> form we should resize; if not specified, we default to
		/// <see cref="SelectedTab" />.</param>
		public void ResizeTabContents(FireTitleTab tab = null)
		{
			if (tab == null)
			{
				tab = SelectedTab;
		...
		/// <summary>Forwards a message received by <see cref="FireTitleTabsOverlay" /> to the underlying window.</summary>
		...
		{
			foreach (FireTitleTab tab in Tabs.Where(tab => tab.Content.Handle == e.WindowHandle))
			{
		...
		private void _tabs_CollectionModified(object sender, ListModificationEventArgs e)
		{
			...
			if (e.Modification == ListModification.ItemAdded || e.Modification == ListModification.RangeAdded)
			{
				...
				{
					FireTitleTab currentTab = Tabs[i + e.StartIndex];

					currentTab.Content.TextChanged += Content_TextChanged;
					currentTab.Closing += FireTitleTab_Closing;
		...
		protected virtual TabbedThumbnail CreateThumbnailPreview(FireTitleTab tab)
		{
			TabbedThumbnail preview = TaskbarManager.Instance.TabbedThumbnail.GetThumbnailPreview(tab.Content);
		...
		public virtual void UpdateThumbnailPreviewIcon(FireTitleTab tab, Icon icon = null)
		{
			if (!AeroPeekEnabled)
			{
				return;
		...
		/// Aero Peek preview.
		/// </summary>
		/// <param name="sender">Object from which this event originated (the <see cref="FireTitleTab.Content" /> object in this case).</param>
		/// <param name="e">Arguments associated with the event.</param>
		...
		/// <summary>
		/// Event handler that is called when a tab's <see cref="FireTitleTab.Closing" /> event is fired, which removes the tab from <see cref="Tabs" /> and
		/// re-renders <see cref="_overlay" />.
		/// </summary>
		/// <param name="sender">Object from which this event originated (the <see cref="FireTitleTab" /> in this case).</param>
		/// <param name="e">Arguments associated with the event.</param>
		private void FireTitleTab_Closing(object sender, CancelEventArgs e)
		{
            if (e.Cancel) return;

            FireTitleTab tab = (FireTitleTab) sender;
			CloseTab(tab);
		...
		/// <summary>
		/// Calls <see cref="FireTitleTabsOverlay.Render(bool)"/> on <see cref="_overlay"/> to force a redrawing of the tabs.
		/// </summary>
		...
		protected virtual void CloseTab(FireTitleTab closingTab)
		{
        ...
        {
			foreach (FireTitleTab tab in Tabs.ToArray())
            {
				if (tab.Content != null)
                {
