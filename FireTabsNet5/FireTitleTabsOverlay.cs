...
{
	/// <summary>
	/// Borderless overlay window that is moved with and rendered on top of the non-client area of a  <see cref="FireTitle" /> instance that's responsible
	/// for rendering the actual tab content and responding to click events for those tabs.
	/// </summary>
	public class FireTitleTabsOverlay : Form
	{
		...
		protected static Dictionary<FireTitle, FireTitleTabsOverlay> _parents = new Dictionary<FireTitle, FireTitleTabsOverlay>();

		/// <summary>Tab that has been torn off from this window and is being dragged.</summary>
		protected static FireTitleTab _tornTab;

		/// <summary>Thumbnail representation of <see cref="_tornTab" /> used when dragging.</summary>
		protected static FireTabForm _tornTabForm;
		...
		/// window.
		/// </summary>
		protected Tuple<FireTitle, Rectangle>[] _dropAreas = null;
		...
		protected FireTitle _parentForm;
		...
		protected FireTitleTabsOverlay()
		{
		}
		...
		protected FireTitleTabsOverlay(FireTitle parentForm)
		{
		...
		protected FireDisplayType DisplayType
		{
			get
			{
				if (_aeroEnabled)
				{
					return FireDisplayType.Aero;
				}

				if (Application.RenderWithVisualStyles && Environment.OSVersion.Version.Major >= 6)
				{
					return FireDisplayType.Basic;
				}

				return FireDisplayType.Classic;
			}
		}
		...
		public static FireTitleTabsOverlay GetInstance(FireTitle parentForm)
		{
			if (!_parents.ContainsKey(parentForm))
			{
				_parents.Add(parentForm, new FireTitleTabsOverlay(parentForm));
			}
		...
		{
            FormClosing += FireTitleTabOverlay_FormClosing;
        ...
        private void FireTitleTabOverlay_FormClosing(object sender, FormClosingEventArgs e)
        {
        ...
        private void _parentForm_FormClosing(object sender, CancelEventArgs e)
		{
            ...
            FireTitle form = (FireTitle) sender;
		...
		private void ShowTooltip(FireTitle tabsForm, string caption)
		{
		...
		private void ShowTooltipTimer_Elapsed(object sender, System.Timers.ElapsedEventArgs e)
		{
			...
			FireTitleTab hoverTab = _parentForm.TabRenderer.OverTab(_parentForm.Tabs, relativeCursorPosition);

			if (hoverTab != null)
			{
				FireTitle hoverTabForm = hoverTab.Parent;
		...
		protected void InterpretMouseEvents()
		{
			foreach (MouseEvent mouseEvent in _mouseEvents.GetConsumingEnumerable())
			{
				...
				if (nCode >= 0 && (int) WM.WM_MOUSEMOVE == (int) wParam)
				{
					...
					if (_tornTab != null && _dropAreas != null)
					{
						...
						// ReSharper restore ForCanBeConvertedToForeach
						{
							...
							{
								FireTitleTab tabToCombine = null;
									...
									if (!dragArea.Contains(cursorPosition) && _tornTab == null)
									{
										lock (_tornTabLock)
										{
											if (_tornTab == null)
											{
												...
												_tornTabForm = new FireTabForm(_tornTab, _parentForm.TabRenderer);
											}
										}
										...
										if (_tornTab != null)
										{
											...
											_dropAreas = (from window in _parentForm.ApplicationContext.OpenWindows.Where(w => w.Tabs.Count > 0)
														  select new Tuple<FireTitle, Rectangle>(window, window.TabDropArea)).ToArray();
										}
									}
								}));
					}
				...
				else if (nCode >= 0 && (int) WM.WM_LBUTTONUP == (int) wParam)
				{
					...
					if (_tornTab != null)
					{
						FireTitleTab tabToRelease = null;
						...
						if (tabToRelease != null)
						{
							Invoke(
								new Action(
									() =>
									{
										FireTitle newWindow = (FireTitle) Activator.CreateInstance(_parentForm.GetType());
		...
		{
			if (DisplayType == FireDisplayType.Aero)
			{
				return;
			}
			...
			if (DisplayType == FireDisplayType.Basic)
			{
		...
		protected void OnPosition()
		{
			if (!IsDisposed)
			{
				...
				Height = _parentForm.TabRenderer.TabHeight + (DisplayType == FireDisplayType.Classic && _parentForm.WindowState != FormWindowState.Maximized && !_parentForm.TabRenderer.RendersEntireTitleBar
			...
			if (!IsDisposed && _parentForm.TabRenderer != null && _parentForm.WindowState != FormWindowState.Minimized && _parentForm.ClientRectangle.Width > 0)
			{
				...
				using (Bitmap bitmap = new Bitmap(Width, Height, PixelFormat.Format32bppArgb))
				{
					using (Graphics graphics = Graphics.FromImage(bitmap))
					{
						...
						Point offset = _parentForm.WindowState != FormWindowState.Maximized && DisplayType == FireDisplayType.Classic && !_parentForm.TabRenderer.RendersEntireTitleBar
						...
						if (DisplayType == FireDisplayType.Classic && (_parentForm.ControlBox || _parentForm.MaximizeBox || _parentForm.MinimizeBox))
						{
							int boxWidth = 0;
			...
			switch ((WM) m.Msg)
			{
				...
				case WM.WM_LBUTTONDOWN:
					...
					else
					{
						...
						FireTitleTab clickedTab = _parentForm.TabRenderer.OverTab(_parentForm.Tabs, relativeCursorPosition);
				...
				case WM.WM_NCMBUTTONUP:
					...
					{
						// When the user clicks a mouse button, save the tab that the user was over so we can respond properly when the mouse button is released
						FireTitleTab clickedTab = _parentForm.TabRenderer.OverTab(_parentForm.Tabs, relativeCursorPosition2);
		...
		/// <summary>
		/// Contains information on mouse events captured by <see cref="FireTitleTabsOverlay.MouseHookCallback" /> and processed by
		/// <see cref="FireTitleTabsOverlay.InterpretMouseEvents" />.
		/// </summary>
