...
namespace FireTabsNet5
{
    public class WindowsSizingBoxes
    {
        ...
        public WindowsSizingBoxes(FireTitle parentWindow)
        {
            ...
            _closeImage = LoadSvg(Encoding.UTF8.GetString(Resources.Close1), 10, 10);
        ...
        public HT NonClientHitTest(Point Firecursor)
        {
            if (_minimizeButtonArea.Contains(Firecursor))
            {
                return HT.HTMINBUTTON;
            }

            else if (_maximizeRestoreButtonArea.Contains(Firecursor))
            {
                return HT.HTMAXBUTTON;
            }

            else if (_closeButtonArea.Contains(Firecursor))
            {
                return HT.HTCLOSE;
            }
