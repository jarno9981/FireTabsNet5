...
namespace FireTabsNet5
{
	...
	public abstract class FireBaseTabRenderer
	{
		...
		protected Color ForeColor = Color.Orange;
		...
		protected FireTitle _parentWindow;
		...
		protected FireBaseTabRenderer(FireTitle parentWindow)
		{
		...
		{
			ListWithEvents<FireTitleTab> tabs = (ListWithEvents<FireTitleTab>) sender;
		...
		public virtual FireTitleTab OverTab(IEnumerable<FireTitleTab> tabs, Point cursor)
		{
			FireTitleTab overTab = null;

			foreach (FireTitleTab tab in tabs.Where(tab => tab.TabImage != null))
			{
		...
		/// <param name="cursor">Current location of the cursor.</param>
		/// <returns>
		/// True if the <paramref name="cursor" /> is within the <see cref="FireTitleTab.Area" /> of the <paramref name="tab" /> and is over a non- transparent
		/// pixel of <see cref="FireTitleTab.TabImage" />, false otherwise.
		/// </returns>
		protected virtual bool IsOverTab(FireTitleTab tab, Point cursor)
		{
			return IsOverNonTransparentArea(tab.Area, tab.TabImage, cursor);
		}

		/// <summary>Checks to see if the <paramref name="cursor" /> is over the <see cref="FireTitleTab.CloseButtonArea" /> of the given <paramref name="tab" />.</summary>
		/// <param name="tab">The tab whose <see cref="FireTitleTab.CloseButtonArea" /> we are to check to see if it contains <paramref name="cursor" />.</param>
		/// <param name="cursor">Current position of the cursor.</param>
		/// <returns>True if the <paramref name="tab" />'s <see cref="FireTitleTab.CloseButtonArea" /> contains <paramref name="cursor" />, false otherwise.</returns>
		public virtual bool IsOverCloseButton(FireTitleTab tab, Point cursor)
		{
        ...
        protected virtual int GetMaxTabAreaWidth(List<FireTitleTab> tabs, Point offset)
        {
		...
		public virtual void Render(List<FireTitleTab> tabs, Graphics graphicsContext, Point offset, Point cursor, bool forceRedraw = false)
		{
			...
			int i = tabs.Count - 1;
			List<Tuple<FireTitleTab, int, Rectangle>> activeTabs = new List<Tuple<FireTitleTab, int, Rectangle>>();
			...
			if (selectedIndex != -1)
			{
				FireTitleTab selectedTab = tabs[selectedIndex];
				...
				if (IsTabRepositioning && _tabClickOffset != null)
				{
					...
					{
						FireTitleTab tab = tabs[selectedIndex];
				...
				activeTabs.Add(new Tuple<FireTitleTab, int, Rectangle>(tabs[selectedIndex], selectedIndex, tabArea));
			}

			// Loop through the tabs in reverse order since we need the ones farthest on the left to overlap those to their right
			foreach (FireTitleTab tab in ((IEnumerable<FireTitleTab>) tabs).Reverse())
			{
			...
			foreach (Tuple<FireTitleTab, int, Rectangle> tab in activeTabs)
			{
		...
		protected virtual void Render(Graphics graphicsContext, FireTitleTab tab, int index, Rectangle area, Point cursor, Image tabLeftImage, Image tabCenterImage, Image tabRightImage)
		{
			if (_suspendRendering)
			{
				return;
		...
		protected virtual Image GetTabLeftImage(FireTitleTab tab)
		{
			return tab.Active
		...
		protected virtual Image GetTabCenterImage(FireTitleTab tab)
		{
			return tab.Active
				? _activeCenterImage
		...
		protected virtual Image GetTabRightImage(FireTitleTab tab)
		{
			return tab.Active
		...
		/// <summary>
		/// Called when a torn tab is dragged into the <see cref="FireTitle.TabDropArea" /> of <see cref="_parentWindow" />.  Places the tab in the list and
		...
		internal virtual void CombineTab(FireTitleTab tab, Point cursorLocation)
		{
