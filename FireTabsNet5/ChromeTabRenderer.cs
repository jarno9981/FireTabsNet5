...
namespace FireTabsNet5
{
    ...
    public class ChromeTabRenderer : FireBaseTabRenderer
	{
		...
		public ChromeTabRenderer(FireTitle parentWindow)
			: base(parentWindow)
		{
			// Initialize the various images to use during rendering
			_activeLeftSideImage = Resources.Left;
			_activeRightSideImage = Resources.Right;
			_activeCenterImage = Resources.Center;
			_inactiveLeftSideImage = Resources.InactiveLeft;
			_inactiveRightSideImage = Resources.InactiveRight;
			_inactiveCenterImage = Resources.InactiveCenter;
			_closeButtonImage = Resources.Close;
			_closeButtonHoverImage = Resources.CloseHover;
          
			_background = IsWindows10 ? Resources.Background : null;
			_addButtonImage = new Bitmap(Resources.Add);
			_addButtonHoverImage = new Bitmap(Resources.AddHover);
        ...
        public override void Render(List<FireTitleTab> tabs, Graphics graphicsContext, Point offset, Point cursor, bool forceRedraw = false)
        {
        ...
        protected override void Render(Graphics graphicsContext, FireTitleTab tab, int index, Rectangle area, Point cursor, Image tabLeftImage, Image tabCenterImage, Image tabRightImage)
        {
            if (!IsWindows10 && !tab.Active && index == _parentWindow.Tabs.Count - 1)
            {
                tabRightImage = Resources.InactiveRightNoDivider;
            }

            base.Render(graphicsContext, tab, index, area, cursor, tabLeftImage, tabCenterImage, tabRightImage);
        }

        protected override int GetMaxTabAreaWidth(List<FireTitleTab> tabs, Point offset)
        {
