...
using FireApiCodePackShell.Shell.Interop.Common;
...
namespace FireApiCodePackShell.Controls.WindowsForms
{
    ...
    public sealed class ExplorerBrowser :
        ...
        FireApiCodePackShell.Controls.IServiceProvider,
        ...
        IMessageFilter
    {
        ...
        public void Navigate(ShellObject shellObject)
        {
            ...
            {
                var hr = explorerBrowserControl.BrowseToObject(shellObject.NativeShellItem, 0);
                if (hr != HResult.Ok)
                {
        ...
        HResult ICommDlgBrowser3.GetCurrentFilter(StringBuilder pszFileSpec, int cchFileSpec) => HResult.Ok;

        HResult ICommDlgBrowser3.GetDefaultMenuText(IShellView shellView, IntPtr text, int cchMax) => HResult.False;
        ...
        HResult ICommDlgBrowser3.OnDefaultCommand(IntPtr ppshv) => HResult.False;
        ...
        /// <returns></returns>
        HResult FireApiCodePackShell.Controls.IServiceProvider.QueryService(
        ...
        internal IShellItemArray GetItemsArray()
        {
            ...
            if (iFV2 != null)
            {
                try
                {
                    ...
                    var hr = iFV2.Items((uint)ShellViewGetItemObject.AllView, ref iidShellItemArray, out var oArray);
                    if (hr != HResult.Ok &&
                        hr != HResult.Fail &&
        ...
        internal IShellItemArray GetSelectedItemsArray()
        {
            ...
            if (iFV2 != null)
            {
                ...
                {
                    var iidShellItemArray = new Guid(ShellIIDGuid.IShellItemArray);
                    var hr = iFV2.Items((uint)ShellViewGetItemObject.Selection, ref iidShellItemArray, out var oArray);
                    iArray = oArray as IShellItemArray;
                    if (hr != HResult.Ok &&
                        hr != HResult.ElementNotFound &&
                        hr != HResult.Fail)
