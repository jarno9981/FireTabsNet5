...
namespace FireApiCodePackShell.Shell
{
    ...
    public class ShellSearchFolder : ShellSearchCollection
    {
        ...
        /// <summary>
        /// Gets the <see cref="FireApiCodePackShell.Shell.SearchCondition"/> of the search. When this property is not set, the
        ...
        /// <summary>Sets the search folder display name.</summary>
        public void SetDisplayName(string displayName)
        {
            var hr = NativeSearchFolderItemFactory.SetDisplayName(displayName);

            if (!CoreErrorHelper.Succeeded(hr)) { throw new ShellException(hr); }
        }

        /// <summary>
        /// Sets folder logical view mode. The default settings are based on the FolderTypeID which is set by the
        ...
        {
            var hr = NativeSearchFolderItemFactory.SetFolderLogicalViewMode(mode);
        ...
        {
            var hr = NativeSearchFolderItemFactory.SetFolderTypeID(value);
        ...
        {
            var hr = NativeSearchFolderItemFactory.SetGroupColumn(ref propertyKey);

            if (!CoreErrorHelper.Succeeded(hr)) { throw new ShellException(hr); }
        }

        /// <summary>
        /// Sets the search folder icon size. The default settings are based on the FolderTypeID which is set by the
        /// SearchFolder::SetFolderTypeID method.
        /// </summary>
        public void SetIconSize(int value)
        {
            var hr = NativeSearchFolderItemFactory.SetIconSize(value);
        ...
        public void SetStacks(params string[] canonicalNames)
        {
            ...
            foreach (var prop in canonicalNames)
            {
                ...
                var result = PropertySystemNativeMethods.PSGetPropertyKeyFromName(prop, out var propKey);
        ...
        {
            var hr = NativeSearchFolderItemFactory.SetVisibleColumns(value == null ? 0 : (uint)value.Length, value);
        ...
        {
            var hr = NativeSearchFolderItemFactory.SetSortColumns(value == null ? 0 : (uint)value.Length, value);
